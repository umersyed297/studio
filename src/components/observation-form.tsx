...
import { saveObservationAction } from '@/lib/actions';
...
export default function ObservationForm() {
  ...
  const handleImageChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      // Validate with Zod first (for type and size) before processing
      ...
      form.clearErrors("imageFile"); // Clear previous errors if any
      ...
      reader.onloadend = () => {
        ...
        setAiSuggestions([]);
        setAiError(null);
        suggestSpeciesNames({ imageDataUri: reader.result as string })
          .then(response => {
  ...
  // Effect to re-validate imageFile when imagePreview changes, ensuring RHF is aware
  useEffect(() => {
  ...
  const onSubmit: SubmitHandler<ObservationFormValues> = async (data) => {
    const result = await saveObservationAction(data, aiSuggestions);
    if (result.success) {
      toast({
        title: 'Success!',
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <FormField
              control={form.control}
              name="imageFile"
              render={({ field: { onChange, value, ...restField }}) => ( // Destructure onChange to use custom handler
                <FormItem>
                  ...
                  <div className="relative">
                     <FormControl>
                        <Input 
                          ...
                          onChange={(e) => {
                            onChange(e.target.files); // Update RHF's internal state
